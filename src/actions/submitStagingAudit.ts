  ...
  try {
    ...
    await resend.emails.send({
      ...
      subject: isApproved 
        ...
        : `Tweak List Received: ${payload.businessName}`,
      html: `
        <!DOCTYPE html>
        <html>
                  ...
                  <tr>
                    <td style="padding: 40px;">
                      <h2 style="margin: 0 0 16px 0; color: #0f172a; font-size: 18px; font-weight: 700;">
                        ${isApproved ? 'I have received your official sign-off!' : 'I have received your requested tweaks!'}
                      </h2>
                      ...
                      <div style="background-color: ${isApproved ? '#f0fdf4' : '#fdf4ff'}; border: 1px solid ${isApproved ? '#bbf7d0' : '#f0abfc'}; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
                        ...
                        <p style="margin: 8px 0 0 0; color: ${isApproved ? '#15803d' : '#a21caf'}; font-size: 14px; line-height: 1.5;">
                          ${isApproved
                            ...
                            : 'I will apply your requested copy tweaks and layout adjustments. Once polished, I will notify you so I can get your professional domain live!'}
                        </p>
                      </div>
    ...
    await resend.emails.send({
      ...
      subject: isApproved 
        ...
        : `🛠️ TWEAKS REQUESTED: ${payload.businessName}`,
      html: `
        <!DOCTYPE html>
        <html>
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; padding: 20px; color: #111827;">
          <div style="max-w: 600px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 8px; padding: 24px; background: #ffffff;">
            <h2 style="color: ${isApproved ? '#059669' : '#c026d3'}; margin-top: 0;">
              ${isApproved ? '✅ Client Walkthrough Approved!' : '🛠️ Client Requested Build Tweaks'}
            </h2>
            ...
            <h3 style="font-size: 16px; margin-bottom: 8px;">Logged Section Tweaks / Notes:</h3>
            ...
            <p style="font-size: 14px; color: #4b5563;">
              <strong>Action Required:</strong> ${isApproved 
                ...
                : 'Review the copy tweaks above, apply them to the codebase, and notify the client once polished!'}
            </p>
          </div>
        </body>
        </html>
