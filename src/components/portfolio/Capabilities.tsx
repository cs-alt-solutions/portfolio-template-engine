...
export default function Capabilities({ 
  ...
  // 🚀 SINGLE SOURCE OF TRUTH: Explicitly define geometry for each vibe!
  const getCardStyles = () => {
    if (themeStyle === 'elegant') {
      return 'bg-white border border-zinc-200 hover:border-zinc-400 hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] rounded-sm p-10 transition-all hover:-translate-y-1';
    }
    if (themeStyle === 'organic') {
      return 'bg-[#EAE5D9] border border-[#4A5D4E]/10 hover:border-[#4A5D4E]/40 hover:shadow-xl rounded-[30px] p-10 transition-all hover:-translate-y-1';
    }
    if (themeStyle === 'editorial') {
      return 'bg-transparent border-t-2 border-b-2 border-black hover:bg-black/5 rounded-none p-10 transition-all';
    }
    if (isNeo) return 'bg-white border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-1 hover:shadow-none rounded-none p-10 transition-all';
    if (isIndustrial) return 'bg-zinc-900 border-2 border-zinc-700 shadow-[8px_8px_0_0_rgba(0,0,0,0.5)] hover:-translate-y-1 rounded-none p-10 transition-all';
    if (isMidnight) return 'bg-zinc-900/30 backdrop-blur-2xl border border-white/5 shadow-2xl hover:bg-zinc-900/40 hover:shadow-[0_0_30px_rgba(255,255,255,0.05)] hover:-translate-y-1 rounded-2xl p-10 transition-all';
    if (isCyber) return `bg-black/60 border border-white/10 hover:border-${brandColor} hover:shadow-[0_0_30px_rgba(var(--${brandColor}),0.2)] rounded-none p-10 transition-all`;
    if (isLightMode) return 'bg-white/50 border border-stone-200 hover:border-stone-300 hover:shadow-2xl rounded-3xl hover:-translate-y-1 p-10 transition-all';
    
    // Fallback Dark
    return 'bg-zinc-900/60 border border-zinc-800 hover:border-zinc-700 hover:bg-zinc-900/80 hover:shadow-2xl rounded-3xl hover:-translate-y-1 p-10 transition-all';
  };

  const getMiniCardStyles = () => {
    if (themeStyle === 'elegant') return 'bg-white border border-zinc-200 hover:shadow-md rounded-sm';
    if (themeStyle === 'organic') return 'bg-[#EAE5D9] border border-[#4A5D4E]/10 hover:shadow-md rounded-[20px]';
    if (themeStyle === 'editorial') return 'bg-transparent border-b border-black rounded-none';
  ...
  const isSharpTheme = isIndustrial || isNeo || isCyber || themeStyle === 'editorial' || themeStyle === 'elegant';
    ...
    <div className="w-full max-w-6xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {items.map((item, i) => {
          ...
          if (isBulletOnly && (!item.bullets || item.bullets.length === 0)) {
            return (
              ...
              >
                ...
                {hasProof && (
                  <span className={`inline-flex items-center gap-1 text-[10px] font-mono font-bold uppercase tracking-widest px-2.5 py-1 ${isSharpTheme ? 'rounded-none' : 'rounded-full'} border ${isLightMode ? 'bg-zinc-100 border-zinc-300 text-zinc-700' : 'bg-white/10 border-white/20 text-white'}`}>
          ...
          return (
            <div 
              key={i} 
              className={`flex flex-col justify-between ${getCardStyles()}`}
            >
              <div>
      ...
      {activeModalService && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8 bg-black/85 backdrop-blur-md animate-in fade-in zoom-in-95 duration-200">
          <div className="bg-zinc-950 border border-zinc-800 rounded-sm w-full max-w-5xl max-h-[85vh] flex flex-col overflow-hidden shadow-2xl relative">
            <div className="flex items-center justify-between p-6 border-b border-zinc-800 bg-zinc-900/50">
              <div>
                <span className={`text-[10px] font-mono uppercase tracking-widest px-2.5 py-1 rounded-sm bg-white/10 border border-white/20 text-white`}>
                  Service Portfolio
                </span>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {activeModalService.images.map((img: GalleryItem, i: number) => (
                  <div key={i} className="group relative aspect-square rounded-sm overflow-hidden bg-zinc-900 border border-zinc-800">
            ...
            <div className="p-4 border-t border-zinc-800 bg-zinc-900/30 flex justify-end">
              <button
                onClick={() => setActiveModalService(null)}
                className="px-6 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-white font-bold text-xs uppercase tracking-widest rounded-sm transition-colors"
              >
                Close Gallery
              </button>
            </div>
