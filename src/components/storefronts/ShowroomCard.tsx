...
export default function ShowroomCard({ site }: { site: StorefrontData }) {
  ...
  return (
    <Link 
      href={targetUrl}
      {...linkProps}
      className={`group relative flex flex-col w-full rounded-3xl overflow-hidden border border-white/5 bg-black/40 backdrop-blur-xl transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_-10px_rgba(0,0,0,0.5)] ${isOffline ? 'opacity-50 grayscale' : ''}`}
      style={{ 
        // We use a CSS variable to pass the dynamic hex color to our hover states safely
    ...
    >
      ...
      <div className="relative h-64 w-full bg-zinc-950/80 flex items-center justify-center overflow-hidden border-b border-white/5">
        ...
        <div 
          className="absolute inset-0 opacity-10 blur-3xl group-hover:opacity-40 transition-opacity duration-700"
